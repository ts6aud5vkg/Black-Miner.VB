...
Namespace Software
	...
	Public Partial Class demo
		...
		' Token: 0x06000013 RID: 19 RVA: 0x00005848 File Offset: 0x00003A48
		Private Sub demo_Load(sender As Object, e As EventArgs)
			Me.GetBitcoinExchangeRate()
		End Sub

		' Token: 0x06000014 RID: 20 RVA: 0x00005854 File Offset: 0x00003A54
		Private Function IsAtLeastOneCheckBoxChecked() As Boolean
			Return Me.checkBox1.Checked OrElse Me.checkBox2.Checked OrElse Me.checkBox3.Checked OrElse Me.checkBox4.Checked OrElse Me.checkBox5.Checked OrElse Me.checkBox6.Checked
		End Function

		' Token: 0x06000015 RID: 21 RVA: 0x000058B0 File Offset: 0x00003AB0
		Private Sub btnStart_Click(sender As Object, e As EventArgs)
			Dim <btnStart_Click>d__ As demo.<btnStart_Click>d__15
			<btnStart_Click>d__.<>t__builder = AsyncVoidMethodBuilder.Create()
			<btnStart_Click>d__.<>4__this = Me
			<btnStart_Click>d__.<>1__state = -1
			<btnStart_Click>d__.<>t__builder.Start(Of demo.<btnStart_Click>d__15)(<btnStart_Click>d__)
		End Sub

		' Token: 0x06000016 RID: 22 RVA: 0x000058E7 File Offset: 0x00003AE7
		Private Sub btnStop_Click(sender As Object, e As EventArgs)
			Me.cancellationTokenSource.Cancel()
			Environment.[Exit](0)
		End Sub

		' Token: 0x06000017 RID: 23 RVA: 0x000058FC File Offset: 0x00003AFC
		Private Sub StartMining(token As CancellationToken)
			Dim random As Random = New Random()
			Me.threshold = random.[Next](30000, 1000000)
			While Not token.IsCancellationRequested
				Dim words As String = Me.GenerateRandomWords()
				MyBase.Invoke(New Action(Sub()
					Me.textBoxOutput.AppendText(words + vbLf)
					Me.tries += 1
					Me.DisplayCounter(Me.tries)
					Me.messageCounter += 1
					If Me.messageCounter >= Me.threshold Then
						Dim randomValue As Double = random.NextDouble() * 0.01111 + 0.00123
						Dim btcValue As Double = Me.GetBitcoinExchangeRate()
						Dim result As Double = randomValue * btcValue
						Me.textBox2.AppendText(String.Format("btc {0:F5} = ${1:F2} | LOCKED", randomValue, result) + Environment.NewLine)
						Me.linesCount += 1
						Me.DisplayLinesCounter(Me.linesCount)
						Me.messageCounter = 0
						Me.threshold = random.[Next](30000, 1000000)
					End If
				End Sub))
		...
		' Token: 0x06000018 RID: 24 RVA: 0x0000597A File Offset: 0x00003B7A
		Private Sub textBoxOutput_TextChanged(sender As Object, e As EventArgs)
		End Sub

		' Token: 0x06000019 RID: 25 RVA: 0x0000597C File Offset: 0x00003B7C
		Private Sub DisplayCounter(counter As Integer)
			Me.lblCounter.Text = String.Format("Checked: {0}", counter)
		End Sub

		' Token: 0x0600001A RID: 26 RVA: 0x00005999 File Offset: 0x00003B99
		Private Sub button1_Click_1(sender As Object, e As EventArgs)
			New info().Show()
		End Sub

		' Token: 0x0600001B RID: 27 RVA: 0x000059A5 File Offset: 0x00003BA5
		Private Sub DisplayLinesCounter(counter As Integer)
			Me.label2.Text = String.Format("Found: {0}", counter)
		End Sub

		' Token: 0x0600001C RID: 28 RVA: 0x000059C2 File Offset: 0x00003BC2
		Private Sub checkBox1_CheckedChanged(sender As Object, e As EventArgs)
		End Sub

		' Token: 0x0600001D RID: 29 RVA: 0x000059C4 File Offset: 0x00003BC4
		Private Function GetBitcoinExchangeRate() As Double
			Dim result As Double
			Try
				Dim API_KEY As String = "AJ1U0MIYHQBIG3U9"
				Dim url As String = "https://www.alphavantage.co/query?function=CURRENCY_EXCHANGE_RATE&from_currency=BTC&to_currency=USD&apikey=" + API_KEY
				Using wc As WebClient = New WebClient()
					result = Convert.ToDouble(JObject.Parse(wc.DownloadString(url))("Realtime Currency Exchange Rate")("5. Exchange Rate"))
				End Using
			Catch
				MessageBox.Show("You have reached trial limit ", "Error", MessageBoxButtons.OK, MessageBoxIcon.Hand)
				result = 0.0
			End Try
			Return result
		End Function

		' Token: 0x0600001E RID: 30 RVA: 0x00005A58 File Offset: 0x00003C58
		Private Sub BtnDeleteKey_Click(sender As Object, e As EventArgs)
			If File.Exists(demo.activationFilePath) Then
				Try
					File.Delete(demo.activationFilePath)
					MessageBox.Show("Key has been deleted.", "Delete Key", MessageBoxButtons.OK, MessageBoxIcon.Asterisk)
				Catch
				End Try
			End If
		End Sub
